...
paleotopo=Input/Paleotopo_${version}.shp
...
# To change precision of output files (decimal numbers)
awk -F" " '{ printf("%.2f,%.2f,%.2f\n",$1,$2,$3)}' ${final}_${version}.csv > temp.csv
awk -F"," '{ print $1 " " $2 " " $3 }' temp.csv > Output/${final}_${version}${resol}_prec2.csv

cp Output/${final}_${version}${resol}_prec2.csv ../Parameters_maps${resol}/Paleotopo/${final}_${version}${resol}_prec2.csv

## Remove pre-processing files
rm temp.csv
rm ${final}_${version}.csv
rm ${latlon_csv}_${version}.csv
rm ${ptopo_grid}
rm ${out_gmt} 
